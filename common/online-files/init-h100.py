...
def init_pip_source():
    # pip域名解析
    # h100使用的内部pip源解析
    inter_pip_dns = '''172.24.162.98 pypi.tsinghua.mirrors.com apt.tsinghua.mirrors.com'''

    with open("/etc/hosts", "r") as fo:
        lines = fo.readlines()
    # 检查行是否已经存在
    if inter_pip_dns not in lines and inter_pip_dns + '\n' not in lines:
        # 如果不存在，打开文件并追加行
        with open("/etc/hosts", 'a') as fo:
            fo.write(inter_pip_dns + '\n')
    # pip源更新
