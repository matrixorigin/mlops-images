...
function init_pip_source() {
    # pip域名解析
    # h100使用的内部pip源解析
    local inter_pip_dns="172.24.162.98 pypi.tsinghua.mirrors.com apt.tsinghua.mirrors.com"

    # 检查 /etc/hosts 文件是否包含指定的域名解析
    if ! grep -q "$inter_pip_dns" /etc/hosts; then
        echo "$inter_pip_dns" >> /etc/hosts
    fi
